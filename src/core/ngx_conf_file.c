...
ngx_conf_param(ngx_conf_t *cf)
{
    ...
    if (param->len == 0) {
        ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cf->cycle->log, 0,
                       "param-len=%d", param->len);
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_EVENT, cf->cycle->log, 0,
                   "b: pos=%s last=%s file_pos=%O file_last=%O start=%s end=%s",
                   b.pos, b.last, b.file_pos, b.file_last, b.start, b.end);
...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    /*
     * 对nginx.conf中的每一行配置，执行ngx_conf_handler
     * ngx_conf_handler 功能：
     *     遍历ngx_conf_module和所有核心模块（此处是核心模块，因为cf上下文中指定模块类型为NGX_CORE_MODULE），
     *     调用模块中与配置行匹配的函数，以实现配置存入模块结构体
     */
    for ( ;; ) {
